        ...
        with open(file_path, 'r') as file:
            ...
            if occurrences > 0:
                ...
                for index in indices:
                    ...
                    if 0 < x_value <= 1920 and 0 < y_value <=1080:
...
def get_patch_indices(position_fixations, img_size, patch_size):
    ...
    for (x, y) in position_fixations:
        ...
        j = x // patch_size
        if i != 14:
            patch_idx = int(i * (img_size // patch_size) + j)
        else:
            patch_idx = int((i - 1) * (img_size // patch_size) + j)
            ...
            if last_folder(root) == 'image':
                ...
                for fname in sorted(fnames):
                    for txt in sorted(os.listdir(txt_dir_path)):
                        ...
                        position_fixations = scale_position_fixations(position_fixations, (1920, 1080), (224, 224))
                        patch_indices = get_patch_indices(position_fixations, 224, 16)
                        path = os.path.join(root, fname)
                        if is_valid_file(path):
                            item = path, patch_indices, class_index
...
class DatasetFolder(VisionDataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any, Any, Any]:
        ...
        while True:
            try:
                path, patch_indices, target = self.samples[index]
        ...
        if self.transform is not None:
            sample = self.transform(sample, self.samples[index])
