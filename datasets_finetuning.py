...
from fix_masking_generator import FixMaskingGenerator
from dataset_folder_finetuning import ImageFolder
...
class DataAugmentationForBEiT(object):
    def __init__(self, args):
        ...
        self.masked_position_generator = FixMaskingGenerator(args.window_size)

    def __call__(self, image, path):
        for_patches, for_visual_tokens = self.common_transform(image)
        return \
            self.patch_transform(for_patches), self.visual_token_transform(for_visual_tokens), \
            self.masked_position_generator(path)
...
def build_dataset(is_train, is_transform, args):
    transform = build_transform(is_transform, args)
    ...
    if args.data_set == 'CIFAR':
        ...
        dataset = ImageFolder(root, data_type, transform=transform)
...
def build_transform(is_transform, args):
