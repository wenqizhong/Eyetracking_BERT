...
from datasets_finetuning import build_dataset
...
def get_args():
    ...
    # parser.add_argument('--disable_eval_during_finetuning', default=True)
    ...
    parser.add_argument('--clip_grad', type=float, default=0.5, metavar='NORM',
    ...
    # * Finetuning params
    parser.add_argument('--finetune', default='./output_dir/beit_pt_G1+G2.pth',
...
def main(args, ds_init):
    ...
    dataset_train, args.nb_classes = build_dataset(is_train=True, is_transform=False, args=args)

    if args.disable_eval_during_finetuning:
        dataset_val = None
    else:
        dataset_val, _ = build_dataset(is_train=False, is_transform=False, args=args)
    ...
    if mixup_fn is not None:   
        ...
        criterion_cls = LabelSmoothingCrossEntropy(smoothing=args.smoothing)
        criterion_mse = nn.MSELoss()
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, criterion_cls, criterion_mse, data_loader_train, optimizer,
        ...
        if data_loader_val is not None:
            test_stats = evaluate(data_loader_val, model, device, epoch, args.output_dir)
            print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
            ...
            if log_writer is not None:
                ...
                log_writer.update(test_AUC1=test_stats['AUC1'], head="perf", step=epoch)
                log_writer.update(test_sensitivity1=test_stats['sensitivity1'], head="perf", step=epoch)
                log_writer.update(test_specificity1=test_stats['specificity1'], head="perf", step=epoch)
